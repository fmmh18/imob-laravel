        ...
        <div class="card">
            <div class="card-body">
                {!! Form::open([
                    'method' => 'get',
                    'autocomplete' => false,
                    'route' => ['dashboard.feature.index', http_build_query(Request::input())],
                ...
                <div class="col-12 text-end pt-3">
                    <a href="{!! route('dashboard.feature.create') !!}" class="btn btn-success rounded-0">Adicionar</a>
                    <button type="submit" class="btn btn-primary rounded-0"> Pesquisar</button>
                    <a href="{!! route('dashboard.feature.index') !!}" class="btn btn-secondary rounded-0">Limpar</a>
                </div>
                {!! Form::close() !!}
            </div>
        </div>
                        ...
                        <tbody>
                            @forelse ($data as $item)
                                <tr>
                                    <td>{!! $item->id !!}</td>
                                    <td>{!! $item->name !!}</td>
                                    <td><a href="{!! route('dashboard.feature.show', $item->id) !!}" class="btn btn-warning"><i
                                                class="bi bi-pencil-square"></i></a></td>
                                    <td><a href="{!! route('dashboard.feature.delete', $item->id) !!}" class="btn btn-danger btn-delete"><i
